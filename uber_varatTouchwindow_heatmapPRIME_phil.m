...
for p=25:length(U)
 close all;
    ...
    gaussFilt = [1.2]; %sigma filter for imgaussfilt (can be single value for filtering across rows and columns or two values [x y] for filter of row/column
...
% %     XstepSize = 25; %step size for x values AUTO SET BY MATLAB
    ...
    for g = 1:length(comp)
        ...
        V.bounds = [{[-99.5:.5:99.5]} {[-9750:50:9750]} {[-99.5:.5:99.5]} {[-99.5:.5:99.5]} {linspace(-pi,pi,180)} {[-.95:.005:.95]}];
