function [Gprelixtimes,GprelixtimesGO,GprelixtimesNOGO,Gprelixpcth,Gprelixduration,touchespredecision] = assist_predecisionVar_vOLD(array,rec)
...
%length of each of those touches, #mean number of touches. All outputs are in a cell array{rec} with each
%cell being 1 trial.

Gprelixpcth=cell(1,array{rec}.k);
Gprelixtimes=cell(1,array{rec}.k);
Gprelixduration=cell(1,array{rec}.k);

for k =1:array{rec}.k %for each trial
    postsamp=array{rec}.meta.poleOnset(k)+.95; %pole onset time + time to be touchable (.2s) + sampling period (.75s)
    lixtmp=find(array{rec}.S_ctk(16,:,k)==1);
    lix=lixtmp(find(lixtmp>=postsamp*1000,1,'first')); %find first lick after sampling period
    %lix=find(array{rec}.S_ctk(16,:,k)==1,1,'first');%first lick in that trial
    alltouches=[find(array{rec}.S_ctk(9,:,k)==1), find(array{rec}.S_ctk(12,:,k)==1)]; %all touches in trial
    ...
    duration = [find(array{rec}.S_ctk(10,:,k)==1) , find(array{rec}.S_ctk(13,:,k)==1)] - alltouches;
    ...
    if array{rec}.meta.trialCorrect(k)==1 %adding a row below all durations for trial correct or not
...
GprelixtimesGO = Gprelixtimes(array{rec}.meta.trialType);
GprelixtimesNOGO = Gprelixtimes(logical(1-array{rec}.meta.trialType));
Gprelixpcth = Gprelixpcth; %cell array{rec} w/ time between touches
Gprelixduration = Gprelixduration; %cell array{rec} w/ time for each touch
