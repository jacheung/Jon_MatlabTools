...
parfor n=1:length(traces)
    ...
    system(['C:\Users\jacheung\Desktop\bin\trace ' traces(n).name ' ' outputFileName])
...
parfor n=1:length(measures)
    ...
    system(['C:\Users\shires\Desktop\bin\measure ' '--face ' 'top ' measures(n).name ' ' outputFileName '.measurements']);
...
parfor n=1:length(classes)
    ...
    system(['C:\Users\shires\Desktop\bin\classify ' classes(n).name ' ' outputFileName '.measurements ' 'top ' '--px2mm 0.033' ' -n ' trackingInfo{4}] '--limit5.0:25.0');
...
% parfor n=1:length(classes)
%     [~, outputFileName] = fileparts(classes(n).name);
%     system(['C:\Program Files (x86)\WhiskerTracking\bin\reclassify ' classes(n).name ' ' outputFileName '.measurements' ' ' '-n ' trackingInfo{4}]);
