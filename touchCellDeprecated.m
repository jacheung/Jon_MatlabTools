...
function [touchORnaw,peak] = touchCellDeprecated(U,stdVal,exceedSTDthresh)

baselinewindows = [25:51];
...
for g = 1:sum(touchORnaw==-1)
    avgSpikes = sum(touchONN{INHIBtc(g)})/size(touchONN{INHIBtc(g)},1);
    INHIBheatSpks(g,:) = normalize_var(avgSpikes(25:100),0,1);
end
[~,idx]  = min(INHIBheatSpks(:,25:end)');
[peak,INHIBftidx] = sort(idx);
