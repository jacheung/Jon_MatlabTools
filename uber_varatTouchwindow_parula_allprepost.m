...
for p=36
    ...
    cellcode = txt{p};
    [varspikes, prevarspikes,postvarspikes] = assist_varAtTouch(U{rec},[-25:50]);
    % First 6 columns will be values for the variables corresponding to
    % U.varNames
    % Last columns will be the spikes around your given window
    comp={varspikes prevarspikes postvarspikes};
    
    max(smooth(mean(comp{2}(:,7:end))))
    for g = 1:length(comp)
        %%
        %Plot theta at touch
        
        [sorted ,sortedBy ,binBounds]=binslin(comp{g}(:,1),comp{g}(:,7:82),'equalE',41,-100,100);
        thetatouchspks=zeros(length(sorted),79);
            xval=mean([binBounds;[binBounds(2:end) 0]],1);
        thetatouchspks(:,1)=xval(1:end-1);
        ...
        %% Plot amplitude
        [ampsorted ampsortedBy ampbinBounds]=binslin(comp{g}(:,3),comp{g}(:,7:82),'equalX',13);
        %amprange=[1:2:99];
        ...
        %% setpoint
        % [spsorted spsortedBy spbinBounds]=binslin(comp{g}(:,3),comp{g}(:,7:82),'equalE',101,-100,100);
        % sprange=[-99:2:99];
        [spsorted spsortedBy spbinBounds]=binslin(comp{g}(:,4),comp{g}(:,7:82),'equalX',13);
        ...
        %% phase
        [phasesorted phasesortedBy phasebinBounds]=binslin(comp{g}(:,5),comp{g}(:,7:82),'equalX',13);
        ...
        %% max curvature
        [kappasorted kappasortedBy kappabinBounds]=binslin(comp{g}(:,6),comp{g}(:,7:82),'equalX',13);
        ...
        [pvelsorted pvelsortedBy pvelbinBounds]=binslin(comp{g}(:,2),comp{g}(:,7:82),'equalE',41,-10000,10000);
