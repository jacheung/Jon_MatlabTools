...
function [P] = findPeaksTroughs(array,ampthresh,varargin)
...
%% Find max theta excursion on each whisk cycle
    ...
    peakIdx = nan(size(amp_mask));
    troughIdx = nan(size(amp_mask)); 
    ...
    peakIdx(unique(peakphases+maxidx')) = 1; 
    troughIdx(unique(troughphases+minidx')) = 1; 
    
    P.peakIdx=peakIdx; %this is the idx for peak protraction 
    P.troughIdx = troughIdx; %this is the idx for trough of protraction    
