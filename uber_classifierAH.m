...
L.meta.poleOffset = zeros(L.k,1);
L.meta.poleOnset = zeros(L.k,1);
for d = 1:L.k
    L.meta.poleOnset(d) = find(L.S_ctk(15,:,d)==1,1,'first')/1000;
    L.meta.poleOffset(d) = find(L.S_ctk(15,:,d)==1,1,'last')/1000;
end
clear V F1G RMSEgroup F1Gtree RMSEdecomp
% [V] = classifierWrapper(U);
[V] = classifierWrapperAHtmp(L);
%% PARAMETERS SETTING
...
%     [DmatX, DmatY, motorX] = designMatrixBuilder(V(rec),U{rec},designvars,classes,normalization,removal,balance,nanMethod);
    [DmatX, DmatY, motorX] = designMatrixBuilder(V,U,designvars,classes,normalization,removal,balance,nanMethod);
