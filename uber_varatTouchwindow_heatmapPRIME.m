...
%%
winAroundTouch = [-25:50];
popV = touchFeatureBinned(U,winAroundTouch);
...
for d = 1:length(selectedCells)
    ...
    toKeep = diff(popV{currCell}.theta.range)<10; 
    if ~isempty(find(toKeep==0))
        toKeep(find(toKeep==0):end) = 0;
    end
    
    toKeep = logical(ones(length(popV{currCell}.theta.range),1)); 
    
    rawHeat = popV{currCell}.(fields{1}).spikes(toKeep,:);
    smoothedHeat = imgaussfilt(popV{currCell}.(fields{1}).spikes(toKeep,:),gaussFilt,'padding','replicate'); 
    heatToPlot = smoothedHeat; 
    
    figure(322);subplot(plotrow,plotcolumn,d);
    imagesc(heatToPlot);
    colormap(gca,parula);
    set(gca,'Ydir','normal','ytick',1:sum(toKeep),'yticklabel',[popV{currCell}.(fields{1}).range(toKeep)],...
        'xtick',(0:25:length(window)),'xticklabel',[min(window):25:max(window)],'xlim',[0 length(window)]);
    for k=1:size(popV{currCell}.(fields{1}).counts(toKeep),1)
        text(20,k,num2str(popV{currCell}.(fields{1}).counts(k)),'FontSize',8,'Color','white')
    end
    hold on;plot([sum(window<0) sum(window<0)],[length(popV{currCell}.(fields{1}).range) 0],'w:')
    axis square
    caxis([0 prctile(heatToPlot(:),99)]) %99th percentile cut off so that outliers don't alias signal
    sampledRange(d) = max([popV{currCell}.(fields{1}).range(toKeep)]) - min([popV{currCell}.(fields{1}).range(toKeep)]);
...
for d = 1:length(selectedCells)
    ...
    counts = popV{currCell}.(fields{1}).counts;
    ranges = popV{currCell}.(fields{1}).range ;
    thetavect = [];
    ...
    allspikes = cell2mat(popV{currCell}.(fields{1}).raw);
...
for g = 1:length(bins)
    for d = 1:length(selectedCells)
        ...
        counts = popV{currCell}.theta.counts;
...
for d = 1:length(popV)
    ...
    imagesc(popV{d}.(fields{1}).spikes)
    colormap(gca,parula);
    set(gca,'Ydir','normal','ytick',(1:length(popV{d}.(fields{1}).range)),'yticklabel',[popV{d}.(fields{1}).range],...
        'xtick',(0:25:length(window)),'xticklabel',[min(window):25:max(window)],'xlim',[0 length(window)]);
    for k=1:size(popV{d}.(fields{1}).counts,1)
        text(20,k,num2str(popV{d}.(fields{1}).counts(k)),'FontSize',8,'Color','white')
    end
    hold on;plot([sum(window<0) sum(window<0)],[length(popV{d}.(fields{1}).range) 0],'w:')
    
    
    figure(31);subplot(plotrow,plotcolumn,d);
    imagesc(imgaussfilt(popV{d}.(fields{1}).spikes,gaussFilt,'padding','replicate'));
