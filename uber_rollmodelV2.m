...
mask = objmask.touch;
dkappas = squeeze(array.S_ctk(6,:,:));
kappas = squeeze(array.S_ctk(17,:,:));
phases = squeeze(array.S_ctk(5,:,:));
prophasemask = nan(size(phases));
retphasemask = nan(size(phases)); 
prophasemask(phases<0) = 1;
retphasemask(phases>0) = 1; 

thetas = squeeze(array.S_ctk(1,:,:));
thetas(thetas<1) = nan;


pkappas = kappas.*prophasemask.*mask;
rkappas = kappas.*retphasemask.*mask;
pthetas = thetas.*prophasemask.*mask;
rthetas = thetas.*retphasemask.*mask;
tnums=ceil(intersect(find(~isnan(pkappas)),find(~isnan(pthetas)))/4000);

%filtering out any trials with nan in kappa/theta
pcombined = [pkappas(:) pthetas(:)];
pnnidx = intersect(find(~isnan(pcombined(:,1))),find(~isnan(pcombined(:,2))));
pcombined=pcombined(pnnidx,:);

rcombined = [rkappas(:) rthetas(:)];
rnnidx = intersect(find(~isnan(rcombined(:,1))),find(~isnan(rcombined(:,2))));
rcombined=rcombined(rnnidx,:);

pcombined = [pcombined tnums];
...
[sorted4, ~ ,~]=binslin(pcombined(:,2),pcombined(:,1),'equalE',51,1,101);
[sortedtmp, ~ ,~]=binslin(pcombined(:,2),pcombined(:,[1 3]),'equalE',51,1,101);
...
%% 
    ...
    scatter(pcombined(:,2), pcombined(:,1),'b.')
    hold on;scatter(rcombined(:,2),rcombined(:,1),'r.')
