...
%     vars = {'timeTotouch','onsetTheta','velocity','decompTime'};
    ...
    for k = 1:length(vars)
        ...
        regMethod = 'lasso';
        lambda = loadLambda;
        lambda = lambda(selLambdaIdx); %inherited from running lassoto get optimal lambda
%         lambda = zeros(length(lambda),1)
        ...
        %% LOG CLASSIFIER
            ...
            for rec = 1:10
                ...
                for u=1:cvKfold
                    ...
                    [thetas,cost,~] = ML_oneVsAll(trainX, trainY, numel(unique(DmatY)), lambda(rec), regMethod);
